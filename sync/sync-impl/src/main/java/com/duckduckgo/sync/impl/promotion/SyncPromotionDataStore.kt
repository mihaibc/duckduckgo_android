...
class SyncPromotionDataStoreImpl @Inject constructor(
    ...
    override suspend fun hasBookmarksPromoBeenDismissed(): Boolean {
        return dataStore.data.map { it[bookmarksPromoDismissedKey] }.firstOrNull() ?: false
    }

    override suspend fun recordBookmarksPromoDismissed() {
        dataStore.edit { it[bookmarksPromoDismissedKey] = true }
    }

    override suspend fun hasPasswordsPromoBeenDismissed(): Boolean {
        return dataStore.data.map { it[passwordsPromoDismissedKey] }.firstOrNull() ?: false
    }

    override suspend fun recordPasswordsPromoDismissed() {
        dataStore.edit { it[passwordsPromoDismissedKey] = true }
    }

    companion object {
        private val bookmarksPromoDismissedKey = booleanPreferencesKey("bookmarks_promo_dismissed")
        private val passwordsPromoDismissedKey = booleanPreferencesKey("passwords_promo_dismissed")
    }
}
